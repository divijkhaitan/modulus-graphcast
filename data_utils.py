...
def add_derived_vars(data: xarray.Dataset) -> None:
  ...
  Raises:
    ValueError if `datetime` or `longitude` are not in `data` coordinates.
  """

  for coord in ("datetime", "longitude"):
...
def add_tisr_var(data: xarray.Dataset) -> None:
  ...
  Raises:
    ValueError if `datetime`, 'latitude', or `longitude` are not in `data` coordinates.
  """

  if TISR in data.data_vars:
    return

  for coord in ("datetime", "latitude", "longitude"):
    if coord not in data.coords:
      raise ValueError(f"'{coord}' must be in `data` coordinates.")
