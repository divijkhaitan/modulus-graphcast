...
_TimestampLike = pd.Timestamp | str | np.datetime64 | int | float
_TimedeltaLike = pd.Timedelta | str | np.timedelta64 | int | float

_JULIAN_YEAR_LENGTH_IN_DAYS = 365.25
_SECONDS_PER_DAY = 24 * 60 * 60

_DEFAULT_INTEGRATION_PERIOD = "1h"
...
class _OrbitalParameters:
    ...
    Attributes:
        theta: Number of Julian years since the J2000 epoch.
        rotational_phase: Phase of Earth's rotation (0 means noon at J2000).
        sin_declination: Sine of the Sun's declination.
        cos_declination: Cosine of the Sun's declination.
    ...
    def __init__(
        ...
        self.theta = theta
        self.rotational_phase = rotational_phase
        self.sin_declination = sin_declination
        self.cos_declination = cos_declination
        self.eq_of_time_seconds = eq_of_time_seconds
        self.solar_distance_au = solar_distance_au


def _get_j2000_days(timestamp: pd.Timestamp) -> float:
    """Converts a timestamp to the number of days since epoch J2000.0."""
    return (timestamp - pd.Timestamp("2000-01-01 12:00:00")) / pd.Timedelta(days=1)
...
_get_integrated_radiation_jitted = functools.partial(
...
def get_toa_incident_solar_radiation(
    ...
    use_jit: bool = False,  # Parameter kept for API consistency, but ignored.
) -> np.ndarray:
    ...
    if tsi_data is None:
        tsi_data = _DEFAULT_TSI_DATA_LOADER()  # Assuming this is defined elsewhere.
    tsi = get_tsi(timestamps, tsi_data)   # Assuming this function is defined.
    fn = _get_integrated_radiation_jitted
...
. It must
    ...
    """
    missing_dims = set(["latitude", "longitude"]) - set(data_array_like.dims)
    if missing_dims:
        raise ValueError(
            f"'{missing_dims}' dimensions are missing in data_array_like."
        )

    missing_coords = set(["datetime", "latitude", "longitude"]) - set(data_array_like.coords)
    if missing_coords:
        raise ValueError(
    ...
    radiation = get_toa_incident_solar_radiation(
        ...
        latitude=data_array_like.coords["latitude"].data,
        longitude=data_array_like.coords["longitude"].data,
        tsi_data=tsi_data,
        integration_period=integration_period,
        num_integration_bins=num_integration_bins,
        use_jit=use_jit,
    ...
    if "time" in data_array_like.dims:
        output = xa.DataArray(radiation, dims=("time", "latitude", "longitude"))
    else:
        output = xa.DataArray(radiation[0], dims=("latitude", "longitude"))
